...
const CourseDetailsPage = async({params}) => {
    ...
    const featuredItems = [
        ...
        { icon: BarChart, label: level || 'Beginner' },
        { icon: BookOpen, label: `${totalLessons || 24} Lessons` },
        { icon: Users, label: `${enrollmentCount || 0} Students` },
    ];
    return (
                    ...
                    <div className="sticky top-24 bg-white/70 backdrop-blur-md p-8 rounded-[2rem] border border-white/20 shadow-2xl space-y-8">
                        <div className="space-y-2">
                            ...
                            <div className="flex items-baseline gap-2">
                                <span className="text-5xl font-black text-blue-600">${price || 'Free'}</span>
                                {price && <span className="text-slate-400 line-through font-bold">$199</span>}
                            </div>
                        </div>

                        <div className="space-y-4">
                            <p className="text-slate-700 font-medium">
                                <strong>Instructor:{instructor}</strong>  Industry Expert
                            </p>
